...
/* Predefined clock setups
   ...
   100 . Custom version of the given setup 1. Hopefully working values.
*/
...
#elif (CLOCK_SETUP == 100)
    #define CPU_XTAL_CLK_HZ                 8000000u /* External crystal */
    #define CPU_INT_CLK_HZ                  32768u   /* Internal osc */
    #define DEFAULT_SYSTEM_CLOCK            40000000u /* Default System clock value */
#endif /* (CLOCK_SETUP == 4) */
...
void SystemInit (void) {
  ...
  OSC->CR = (OSC_CR_OSCEN_MASK | OSC_CR_OSCSTEN_MASK | OSC_CR_OSCOS_MASK | OSC_CR_RANGE_MASK);
  ICS->C1 = (ICS_C1_CLKS(0x00) | ICS_C1_RDIV(0x03)); /* changed C1<1> = disabled internal ref clock */
  ICS->C2 = (0x00); /* changed the whole register to 0, may alt. be 0x10 for power save mode in bypass */
  while((ICS->S & ICS_S_IREFST_MASK) != 0x00u); /* wait for external reference */
  while((ICS->S & ICS_S_CLKST_MASK) != 0x00u); /* wait for fll */
#endif
